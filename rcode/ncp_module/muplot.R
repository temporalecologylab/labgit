...
for(i in 1:6){
  pos.y<-(6:1)[i]
  pos.x<-modoutput[rownameshere[i],"mean"]
  lines(modoutput[rownameshere[i],c("25%","75%")],rep(pos.y,2),col="darkgrey")
  points(pos.x,pos.y,cex=1.5,pch=19,col="darkblue")
  for(spsi in 1:spnum){
    pos.sps.i<-which(grepl(paste0("[",spsi,"]"),rownames(modoutput),fixed=TRUE))[2:4]
    jitt<-(spsi/40) + 0.08
    pos.y.sps.i<-pos.y-jitt
    pos.x.sps.i<-modoutput[pos.sps.i[i],"mean"]
    lines(modoutput[pos.sps.i[i],c("25%","75%")],rep(pos.y.sps.i,2),
